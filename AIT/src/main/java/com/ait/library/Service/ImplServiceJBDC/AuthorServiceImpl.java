...
public class AuthorServiceImpl implements ServiceAuthor {
    ...
    public List<Author> findAll() {
        ...
        for (Map<String, Object> row : rows) {
            ...
            author.setNameAuthor((String) row.get("nameAuthor"));
            author.setNation((String) row.get("nation"));
            author.setInformation((String) row.get("information"));
            result.add(author);
        }
        return result;
    ...
    public void save(Author author) {
        String sql = "  INSERT INTO Author (nameAuthor,nation,information) VALUES " +
                "(?,?,?);";
        jdbcTemplate.update(sql,new Object[]{ author.getNameAuthor(),author.getNation(),author.getInformation()});
    }
    ...
    public void update(Author author) {
        String sql = "UPDATE Author SET nameAuthor=?,information=?,nation =? WHERE idAuthor=?";
        jdbcTemplate.update(sql,new Object[]{author.getNameAuthor(),author.getInformation(),author.getNation(),
                author.getIdAuthor()});
    }
